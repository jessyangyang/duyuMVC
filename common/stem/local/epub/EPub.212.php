...
*/
    function addFile($fileName, $fileId, $fileData, $mimetype) {
        ...
        $fileName = preg_replace('#^[/\.]+#i', "", $fileName);
        $this->zip->addFile($fileData, $this->bookRoot.$fileName);
...
*/
    ...
    function addGuide($option = array())
    {
        ...
        foreach ($option as $key => $value) {
            $this->opf_guide[] = "\t\t<reference href=\"" . $value['href'] . "\" type=\"" . $value['type'] . "\" title=\"" . $value['title'] . "\"/>\n";
        }
    }
...
*/
    function finalize() {
        ...
        if (!empty($this->opf_guide)) {
            if (is_array($this->opf_guide)) {
                $opf = "\n\t<guide>\n";
                foreach ($this->opf_guide as $key => $value) {
                    $opf .=$value;
                }
                $opf .= "\t</guide>\n";
                $this->opf .= $opf;
        ...
        if (mb_detect_encoding($this->opf, 'UTF-8', true) === "UTF-8") {
            $this->zip->addFile($this->opf, $this->bookRoot."content.opf");
        } else {
            $this->zip->addFile(mb_convert_encoding($this->opf, "UTF-8"), $this->bookRoot."content.opf");
        }
        if (mb_detect_encoding($this->ncx, 'UTF-8', true) === "UTF-8") {
            $this->zip->addFile($this->ncx, $this->bookRoot."toc.ncx");
        } else {
            $this->zip->addFile(mb_convert_encoding($this->ncx, "UTF-8"), $this->bookRoot."toc.ncx");
        }
        $this->opf = "";
        $this->ncx = "";
...
*/
    function sendBook($fileName) {
        ...
        if (strripos(strrev($fileName), "bupe.") !== 0) {
            $fileName .= ".epub";
        }
