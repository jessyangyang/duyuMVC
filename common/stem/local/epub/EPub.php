...
use \DublinCore;
use \MarcCode;
use \backend\epub\EPubChapterSplitter;
...
*/
    function __construct() {
        ...
        // include_once("EPub.EPubChapterSplitter.php");
...
*/
    private function initialize() {
        ...
        $this->content = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<container version=\"1.0\" xmlns=\"urn:oasis:names:tc:opendocument:xmlns:container\">\n\t<rootfiles>\n\t\t<rootfile full-path=\"" . $this->bookRoot . "content.opf\" media-type=\"application/oebps-package+xml\" />\n\t</rootfiles>\n</container>\n";

        $this->zip->addFile($this->content, "META-INF/container.xml", 0, NULL, FALSE);
        $this->content = NULL;
        $this->ncx = new Ncx();
        $this->opf = new Opf();
        $this->opf->addItem("ncx", "toc.ncx", Ncx::MIMETYPE);
        $this->chapterCount = 0;
    }
    ...
    */
    function addChapter($chapterName, $fileName, $chapterData, $autoSplit = FALSE, $externalReferences = EPub::EXTERNAL_REF_IGNORE, $baseDir = "") {
        ...
        if (!empty($chapter) && is_string($chapter)) {
            ...
            $this->opf->addItem('chapter'. $this->chapterCount,$fileName,'application/xhtml+xml');
            $this->opf->addItemRef('chapter'. $this->chapterCount);

            $navPoint = new NavPoint($this->decodeHtmlEntities($chapterName), $fileName, "navpoint-" . $this->chapterCount);
            ...
            while ($oneChapter) {
                ...
                $this->opf->addItem('chapter'. $this->chapterCount,$partName,'application/xhtml+xml');
...
*/
    ...
    */
    protected function resolveImage($source, &$internalPath, &$internalSrc, &$isSourceExternal, $baseDir = "", $htmlDir = "", $backPath = "") {
        ...
        if ($imageData !== FALSE) {
            ...
            $internalPath = Zip::getRelativePath("images/" . $internalSrc);
     ...
     * @return Boolean
     */
    function addImages($paths = array(),$baseDir = "")
    {
        ...
        foreach ($paths as $key => $path) {
            ...
            // $internalSrc = $pathData['basename'];
            $internalPath = $internalSrc = "";
            $isSourceExternal = FALSE;
            // $backPath = preg_replace('#[^/]+/#i', "../", $cssDir);
            $is_add = $this->resolveImage($path, $internalPath, $internalSrc, $isSourceExternal, $baseDir);
        }
        return $is_add;
    }

    /**
     * Add Guide to $this->opf_guide
     *
     * @access public
     * @param array $option
     * @return void
     */
    function addGuide($option = array())
    {
        if ($this->isFinalized or !is_array($option)) return FALSE;

        foreach ($option as $key => $value) {
            $this->opf_guide[$key] = "\t\t<reference href=\"" . $value['href'] . "\" type=\"" . $value['type'] . "\" title=\"" . $value['title'] . "\"/>\n";
        }
    }
    ...
    */
    function finalize() {
        ...
        if ($this->EPubMark) {
            $this->opf->addMeta("generator", "EPub (" . self::VERSION . ") by A. Grandt, http://www.duyu.cc");
        }
        ...
        if ($this->EPubMark) {
            $this->ncx->addMetaEntry("dtb:generator", "EPub (" . self::VERSION . ") by A. Grandt, http://www.duyu.cc");
        }
        if (!empty($this->generator)) {
        ...
        $Opf = $this->fixEncoding($this->opf->finalize());
        $ncxFinal = $this->fixEncoding($this->ncx->finalize());

        if (mb_detect_encoding($Opf, 'UTF-8', true) === "UTF-8") {
            $this->zip->addFile($Opf, $this->bookRoot."content.opf");
        } else {
            $this->zip->addFile(mb_convert_encoding($Opf, "UTF-8"), $this->bookRoot."content.opf");
        }
        if (mb_detect_encoding($ncxFinal, 'UTF-8', true) === "UTF-8") {
            $this->zip->addFile($ncxFinal, $this->bookRoot."toc.ncx");
        } else {
            $this->zip->addFile(mb_convert_encoding($ncxFinal, "UTF-8"), $this->bookRoot."toc.ncx");
        }
